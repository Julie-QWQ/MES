        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="出库数量" prop="quantitySalse">
              <el-input-number :min="0.01" v-model="form.quantitySalse" placeholder="请输入出库数量" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="出库批次号" prop="batchCode">
              <el-input v-model="form.batchCode" placeholder="请输入出库批次号" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        warehouseId: [
          { required: true, message: "请选择出库的仓库" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除出库单行编号为"' + lineIds + '"的数据项？').then(function() {
